# Clean Maharajtra data for 

library(tidyverse)
library(haven)
library(here)
library(janitor)

# Load the maha data from the Stata file (coded with the food groups)

# Data for women
maha <- read_dta(here( "data", "raw", "Maharajtra", "maharajtra-wom_child_raw.dta")) %>% clean_names()

# Data for children
summary(maha)
table(maha$survey_day)

fixit: stopped here because it appears there is only one day of intake data Maharajtra, Punjab, and Gujarat
# Red meat=9, processed meat=10

maha_nut <-  maha %>% 
  rename(recall = survey_day, id=subject, age=age_year) %>% 
  group_by(id, recall) %>%
  mutate(red_meat = case_when(fg24==9 ~ food_amount_reported,
                              TRUE ~ 0)) %>% 
  mutate(processed_meat = case_when(fg24==10 ~ food_amount_reported, 
                                    TRUE~0)) %>% 
  summarize(b12 = sum(b12),
            iron = sum(iron),
            zinc = sum(zinc),
            vita = sum(vita),
            calc = sum(calc),
            red_meat = sum(red_meat),
            processed_meat = sum(processed_meat),
            omega_3 = sum(omega_3)) %>% distinct()

# Merge in identifiers, incl age/sex
maha_merge <- maha %>%
  rename( age=age_year, recall=survey_day, id=subject) %>%
  dplyr::select( age, sex, recall, id)

# maha_merge_2 <-  merge(maha_merge, maha_weights, by.all="id", all.x=T)

# Rename and format variables for spade
maha_spade <- maha_nut %>% 
  left_join(maha_merge, by=c("id", "recall")) %>%
  mutate(mday = recall) %>%
  group_by(id) %>%
  ungroup() %>%
  dplyr::select(id, age, sex, mday, b12, iron, zinc, vita, calc, red_meat, processed_meat, omega_3) %>% 
  mutate(id=as.integer(id)) %>% distinct()


# Check for missing or different ages
maha_missings <- maha_spade[is.na(maha_spade$age), ] # shows you the missings
maha_missings   

#No missing ages

#Replace any cases where the ages are different for the same individual

ids_data <- unique(maha_spade$id)
for (idid in ids_data){
  data.id <- maha_spade[maha_spade$id == idid, ]
  if(nrow(data.id) > 1){
    maha_spade[maha_spade$id == idid,"age"] <- 
      min(maha_spade[maha_spade$id == idid,"age"])
  }
}

save(maha_spade, file=here("data", "processed", "Maharajtra"), replace)   



